...
***********************************************************************/
			...
			Ptr<ManagedExpression> ToUnary(const ParsingPair<RegexToken, Ptr<ManagedExpression>>& input)
			{
				...
				{
					CHECK_ERROR(false, L"ToUnary(const ParsingPair<RegexToken, Ptr<ManagedExpression>>>)#Î´Öª²Ù×÷·û¡£");
				}
			...
			Ptr<explrec::ExpLrecBase> ToBinaryLrec(const ParsingPair<RegexToken, Ptr<ManagedExpression>>& input)
			{
				...
				{
					CHECK_ERROR(false, L"ToBinaryLrec(const ParsingPair<RegexToken, Ptr<ManagedExpression>>>)#Î´Öª²Ù×÷·û¡£");
				}
				exp->rightOperand=input.Second();
				return explrec::ToExpLrec(exp);
			}
				...
				Ptr<ManagedExpression>>& input)
			{
				...
				{
					CHECK_ERROR(false, L"ToBinaryEq(const {arsingPair<ParsingPair<Ptr<ManagedExpression>, RegexToken>, Ptr<ManagedExpression>>>)#Î´Öª²Ù×÷·û¡£");
				}
				exp->leftOperand=input.First().First();
				exp->rightOperand=input.Second();
				return exp;
